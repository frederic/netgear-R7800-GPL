 ...
 * $Id: mod_auth.c,v 1.273 2010/01/05 23:17:20 castaglia Exp $
 */
...
static struct in_addr get_netmask(char *ifname)
{
  return get_pa(ifname, SIOCGIFNETMASK);
}

static int net_ftp_access(char *fromip)
{
...
static void log_for_usb_access(int auth_code)
{
  char ipstr[64];
  char *cp = ipstr;

  snprintf(ipstr, sizeof(ipstr), "%s", pr_netaddr_get_ipstr(session.c->remote_addr));
  while (!isdigit(*cp))
    cp++;

  if (!net_ftp_access(cp))
    return;

  if (auth_code < 0)
    syslog(6, "[USB remote access rejected] from %s through FTP,", cp);
  else
    syslog(6, "[USB remote access] from %s through FTP,", cp);
}
#endif
...
#ifdef NETGEAR_GUEST_ACCESS
     /* Based on Netgear request,
      * any user name other than "admin" is considered as a guest account and whatever password entered
      * (if it's supplied from the client) is considered as valid.
      */
      if(!strcmp(user_name,"guest"))
...
#ifdef NETGEAR_GUEST_ACCESS
  /* Based on Netgear request, When there is an authentication required (for FTP, HTTP, or SMB),
   * any user name other than "admin" is considered as a guest account and whatever password entered
   * (if it's supplied from the client) is considered as valid. In this case, the user is granted for access with the "guest" access rights.
   * so when the user name is name guest or admin, FTP should auth it as guest.
   */
  if (strcmp(user,"guest") && strcmp(user,"admin") && strcmp(user, "anonymous"))
        user="guest";
#endif
