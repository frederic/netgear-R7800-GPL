...
insert_playlist(const char * path, char * name)
{
	...
	strncpy(type, safe_strrchr(name, '.')+1, 4);
...
fill_playlists()
{
	...
	for( i=3; i<rows*3; i++ )
	{
		...
		strncpy(type, safe_strrchr(plpath, '.')+1, 4);
...
found:
			...
			else
			{
				...
				if( safe_strchr(fname, '\\') )
				{
					fname = modifyString(fname, "\\", "/");
					goto retry;
				}
				else if( (fname = safe_strchr(fname, '/')) )
				{
					fname++;
					goto retry;
				}
