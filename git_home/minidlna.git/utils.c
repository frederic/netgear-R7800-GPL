...
static inline char *safe_strstr(const char *haystack, const char *needle)
{
	if (!haystack || ! needle)
		return NULL;
...
static inline char *safe_strchr(const char *s, int c)
{
	if (!s)
		return NULL;
	return strchr(s, c);
...
static inline char *safe_strrchr(const char *s, int c)
{
	if (!s)
		return NULL;
	return strrchr(s, c);
...
strstrc(const char *s, const char *p, const char t)
{
	...
	endptr = safe_strchr(s, t);
	if (!endptr)
		return safe_strstr(s, p);
...
escape_tag(const char *tag, int force_alloc)
{
	...
	if( safe_strchr(tag, '&') || safe_strchr(tag, '<') || safe_strchr(tag, '>') || safe_strchr(tag, '"') )
	{
		esc_tag = strdup(tag);
