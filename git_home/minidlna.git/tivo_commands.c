	...
	if( strncmp(class, "item", 4) == 0 )
	{
		...
		if( duration ) {
			strcatf(str, "<Duration>%d</Duration>",
			      atoi(safe_strrchr(duration, '.')+1) + (1000*atoi(safe_strrchr(duration, ':')+1))
			      + (60000*atoi(safe_strrchr(duration, ':')-2)) + (3600000*atoi(duration)));
		}
		if( bitrate ) {
              ...
              int anchorOffset, int recurse, char *sortOrder, char *filter, unsigned long int randomSeed)
{
	...
	if( anchorItem )
	{
		if( safe_strstr(anchorItem, "QueryContainer") )
		{
			strcpy(what, "OBJECT_ID");
			saveptr = safe_strrchr(anchorItem, '=');
			if( saveptr )
