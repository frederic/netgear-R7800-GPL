...
remove_watch(int fd, const char * path)
{
	...
	for( l = w = watches; w; l = w, w = w->next )
	{
		if( strcmp(path, w->path) == 0 )
		{
			ret = inotify_rm_watch(fd, w->wd);
			if (w == watches)
			{
				watches = NULL;
				lastwatch = NULL;
...
inotify_insert_file(char * name, const char * path)
{
	...
	if( !depth )
	{
		...
		DPRINTF(E_WARN, L_SCANNER, "inotify start insert file %s\n", path);
		if( insert_file(name, path, id+2, get_next_available_id("OBJECTS", id)) == 0 ) {
			DPRINTF(E_WARN, L_SCANNER, "end insert file %s, status: successful\n", path);
		} else {
			DPRINTF(E_WARN, L_SCANNER, "end insert file %s, status: failed\n", path);
		}
		sqlite3_free(id);
