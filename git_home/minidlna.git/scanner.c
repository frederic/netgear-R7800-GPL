...
insert_containers(const char *name, const char *path, const char *refID, const char *class, int64_t detailID)
{
	...
	if( safe_strstr(class, "imageItem") )
	{
	...
	else if( safe_strstr(class, "audioItem") )
	{
	...
	else if( safe_strstr(class, "videoItem") )
	{
		static long long last_all_objectID = 0;
	...
	if( strcmp(base, BROWSEDIR_ID) != 0 )
	{
		...
		while( !found )
		{
			...
			sql_exec(db, "INSERT into OBJECTS"
			             ...
			             "VALUES"
			             " ('%s', '%s', %Q, %lld, '%s', '%q')",
			             id_buf, parent_buf, refID, detailID, class, safe_strrchr(dir, '/')+1);
			if( (p = safe_strrchr(id_buf, '$')) )
				*p = '\0';
			if( (p = safe_strrchr(parent_buf, '$')) )
				*p = '\0';
			if( (p = safe_strrchr(refID, '$')) )
				*p = '\0';
			dir = dirname(dir);
		}
		free(dir_buf);
...
insert_file(char *name, const char *path, const char *parentID, int object)
{
	...
	if( strstr(class, "imageItem") )
		media_num->totalImage++;
	else if( strstr(class, "audioItem") )
		media_num->totalAudio++;
	else if( strstr(class, "videoItem") )
		media_num->totalVideo++;
	...
	if( *parentID )
	{
		...
		baseid = safe_strrchr(typedir_parentID, '$');
		if( baseid )
		{
...
ScanDirectory(const char *dir, const char *parent, media_types dir_types)
{
	...
	for (i=0; i < n; i++)
	{
		...
		{
			DPRINTF(E_WARN, L_SCANNER, "insert file %s\n", full_path);
			if( insert_file(name, full_path, (parent ? parent:""), i+startID) == 0 )
			{
				fileno++;
				DPRINTF(E_WARN, L_SCANNER, "result: succeed\n");
			}
			else
				DPRINTF(E_WARN, L_SCANNER, "result: fail\n");
		}
		free(name);
		free(namelist[i]);
	}
	...
	if( !parent )
	{
		DPRINTF(E_WARN, L_SCANNER, _("Scanning %s finished (%llu files, %d PlayList, %d Audio, %d Video, %d Image)!\n"), dir, fileno, media_num->totalPlayList, media_num->totalAudio, media_num->totalVideo, media_num->totalImage);
	}
}
